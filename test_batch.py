...
# Every case used to hardcode 'AlphaDG', a font that is not shipped in Fonts/.
# The old lookup silently served a different TTF while still using the AlphaDG
# character map, so 20 of the 25 cases were producing garbled boards and
# reporting OK. Drive the matrix from the fonts that actually exist instead.
BASE_FONT = _default_board_font_name()
...
def run_batch(input_path: Path, out_dir: Path) -> None:
    ...
    # ── Phase 1: every installed font × 3 symbols, default layout, auto-orient
    ...
    tests.append(dict(
        group='P1_nocoords',
        font=BASE_FONT, layout_idx=2, orient='auto',
    ...
    for orient in ('auto', 'black'):
        tests.append(dict(
            ...
            font=BASE_FONT, layout_idx=2, orient=orient,
    ...
    tests.append(dict(
        ...
        font=BASE_FONT, layout_idx=2, orient='black',
        symbol='square', coords=True, lines_count=3, lines_mode='numbered',
    ))
    ...
    for li in range(len(LAYOUTS)):
        tests.append(dict(
            ...
            font=BASE_FONT, layout_idx=li, orient='auto',
    ...
    for fig_font in (FIGURINE_NAMES or [_default_figurine_font_name()]):
        tests.append(dict(
            group='P5_answers',
            font=BASE_FONT, layout_idx=2, orient='auto',
    ...
    for i, cfg in enumerate(tests, 1):
        ...
        opts = {
            ...
            'figurine_font':   cfg.get('figurine_font', _default_figurine_font_name()),
        }
